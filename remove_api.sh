...
if [ -f "route53_record.json" ] || $force_removal; then
...
# Remove API mapping
if [ -f "api_mapping.json" ] || $force_removal; then
...
# Remove domain name
if [ -f "domain_name.json" ] || $force_removal; then
...
# Remove stage
if [ -f "stage.json" ] || $force_removal; then
  api_id=$(jq -r '.ApiId' api.json)
  echo "Removing stage..."
...
# Remove route
if [ -f "route.json" ] || $force_removal; then
  api_id=$(jq -r '.ApiId' api.json)
  echo "Removing route..."
...
# Remove integration
if [ -f "integration.json" ] || $force_removal; then
  api_id=$(jq -r '.ApiId' api.json)
...
# Remove API
if [ -f "api.json" ] || $force_removal; then
  api_id=$(jq -r '.ApiId' api.json)
  echo "Removing API..."
...
if [ -f "lambda.json" ] || $force_removal; then
...
if [ -f "iam_policy_attachment.json" ] || $force_removal; then
...
# Remove IAM role
if [ -f "role.json" ] || $force_removal; then
