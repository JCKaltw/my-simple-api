...
# Remove route
if [ -n "$api_id" ]; then
  echo "Running: aws apigatewayv2 get-routes --api-id $api_id --query \"Items[?Tags[?Key=='Project' && Value=='my-simple-api']].RouteId\""
  route_id=$(aws apigatewayv2 get-routes --api-id "$api_id" --query "Items[?Tags[?Key=='Project' && Value=='my-simple-api']].RouteId" --output text)
  if [ -n "$route_id" ]; then
    echo "Removing route..."
...
# Remove integration
if [ -n "$api_id" ]; then
  echo "Running: aws apigatewayv2 get-integrations --api-id $api_id --query \"Items[?Tags[?Key=='Project' && Value=='my-simple-api']].IntegrationId\""
  integration_id=$(aws apigatewayv2 get-integrations --api-id "$api_id" --query "Items[?Tags[?Key=='Project' && Value=='my-simple-api']].IntegrationId" --output text)
...
if [ -n "$role_name" ]; then
  ...
  echo "Skipping IAM policy attachment removal as the role does not exist."
fi
